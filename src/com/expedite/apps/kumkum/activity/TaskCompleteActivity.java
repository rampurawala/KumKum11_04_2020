...
//Created By Vijay Solanki 30-03-2020

public class TaskCompleteActivity extends BaseActivity {

    private AdView mAdView;
    private String mStrCurriCulam = "", mStrCurriCulamId = "", mFilename = "", mFilePath = "", bucket = "", secretKey = "", mid = "",
            storefilename = "", mStoreImageFile = "", Tag = "TaskCompleteActivity", SchoolId = "", StudentId = "", Year_Id = "", mStrMsg = "";
    private TextView mTxtCurriculamName, mTxtSelectFile, mTxtSelectedPath, mTxtSubmit, mTxtNoData;
    ...
    private CurriculamResponseListAdapter mCurriculamResponseListAdapter;
    ...
    private void init() {
        try {
            ...
            showBannerAd(mAdView, ActivityNames.TaskCompleteActivity);
            showFullScreenAd(TaskCompleteActivity.this, ActivityNames.TaskCompleteActivity);
            ...
            mRecyclerView.setLayoutManager(new LinearLayoutManager(TaskCompleteActivity.this));
            mCurriculamResponseListAdapter = new CurriculamResponseListAdapter(TaskCompleteActivity.this, mCurriculamResponseList, mStrCurriCulam);
            mRecyclerView.setAdapter(mCurriculamResponseListAdapter);
            ...
            SchoolId = Datastorage.GetSchoolId(TaskCompleteActivity.this);
            StudentId = Datastorage.GetStudentId(TaskCompleteActivity.this);
            Year_Id = Datastorage.GetCurrentYearId(TaskCompleteActivity.this);
            ...
            mImgRemove.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Toast.makeText(TaskCompleteActivity.this, "File Removed", Toast.LENGTH_LONG).show();
                }
            });
                    ...
                    try {
                        ...
                        if (mStrMsg != null && !mStrMsg.equalsIgnoreCase("") && !mStrMsg.isEmpty() ||
                            ...
                            if (mFilename != null && !mFilename.equalsIgnoreCase("")) {
                                ...
                                TransferUtility transferUtility = new TransferUtility(s3, TaskCompleteActivity.this);
                            ...
                            Common.showToast(TaskCompleteActivity.this,"Please Select any File or Enter Any Msg to Submit your response.");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
        ...
        try {
            if (isOnline()) {
                ...
                Call<ParentsProfileListModel> call = ((MyApplication) getApplicationContext()).getmRetrofitInterfaceAppService()
                        .GetSubmitCurriculamResponse(StudentId, SchoolId, Year_Id, mStrCurriCulamId, mStrMsg, mFilename, Constants.PLATFORM);
                    ...
                    public void onResponse(Call<ParentsProfileListModel> call, Response<ParentsProfileListModel> response) {
                        try {
                            ...
                            if (tmp.getResponse().equalsIgnoreCase("1")) {
                                Common.showToast(TaskCompleteActivity.this, tmp.getMessage());
                ...
                Common.showToast(TaskCompleteActivity.this, getString(R.string.msg_connection));
            }
        } catch (Exception ex) {
            mProgressBar.setVisibility(View.GONE);
        ...
        try {
            if (isOnline()) {
                Call<AppService> call = ((MyApplication) getApplicationContext()).getmRetrofitInterfaceAppService()
                        .getStudentCurriculamResponse(StudentId, SchoolId, Year_Id, mStrCurriCulamId, Constants.PLATFORM);
                        ...
                        try {
                            ...
                            if (tmp.getResponse().equalsIgnoreCase("1")) {
                                if (tmp.getListArray() != null && tmp.getListArray().size() > 0) {
                                    ...
                                    mCurriculamResponseListAdapter.notifyDataSetChanged();
                                } else {
                                   /* mTxtNoData.setVisibility(View.VISIBLE);
                                    mRecyclerView.setVisibility(View.GONE);*/
                                }
                                //  Common.showToast(TaskCompleteActivity.this, tmp.getStrResult());
                            } else {
                               /* mTxtNoData.setVisibility(View.VISIBLE);
                                mRecyclerView.setVisibility(View.GONE);
                                Common.showToast(TaskCompleteActivity.this, tmp.getStrResult());*/
                            ...
                            //mProgressBar.setVisibility(View.GONE);
                            Constants.writelog(Tag, "getCurriculamResponse::273" + ex.getMessage());
                        }
                    }

                    @Override
                    public void onFailure(Call<AppService> call, Throwable t) {
                        Constants.writelog(Tag, "getCurriculamResponse:281" + t.getMessage());
                        mProgressBar.setVisibility(View.GONE);
    ...
    private void getFtpDetail() {
        try {
            if (isOnline()) {
                Call<AppService> call = ((MyApplication) getApplicationContext()).getmRetrofitInterfaceAppService()
                        .GetLeavePdfImageFTPDetail(StudentId, SchoolId, Year_Id, "2", String.valueOf(SchoolDetails.appname), BuildConfig.VERSION_CODE + "", Constants.PLATFORM);
                call.enqueue(new Callback<AppService>() {
                    @Override
                    public void onResponse(Call<AppService> call, Response<AppService> response) {
                        try {
    ...
    private void SelectPhoto() {
        try {
            AlertDialog.Builder alertDialog = new AlertDialog.Builder(TaskCompleteActivity.this,
            ...
            final boolean result = Utility.checkPermission(TaskCompleteActivity.this);
                ...
                try {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                        ...
                        if (clipData == null) {
                            ...
                            Uri tempUri = getImageUri(TaskCompleteActivity.this, bitmap);
                           ...
                           /* Picasso.with(TaskCompleteActivity.this).load("file://" + mStoreImageFile)
                    ...
                    if (mFilePath != null && !mFilePath.equals("")) {
                        ...
                        if (mFilename.contains(".pdf")) {
                          ...
                          //  mStoreImageFile=mFilename;
                            ...
                            Common.showToast(TaskCompleteActivity.this, "Please Select PDF.");
                        }
                    } else {
                        Common.showToast(TaskCompleteActivity.this, "Please Select from internal memory.");
                    }
    ...
    public void onBackPressed() {
        try {
            TaskCompleteActivity.this.finish();
            hideKeyboard(TaskCompleteActivity.this);
