...
public class SubmissionFileListAdapter extends RecyclerView.Adapter {
    ...
    public SubmissionFileListAdapter(Activity context, List<CurriculumListModel.FileList> ItemList, AppCompatButton downloadMain, AppCompatButton shareMain, LinearLayout shareDownload) {
        mList = ItemList;
        mContext = context;
        ...
        try {
            if (holder instanceof CustomViewHolder) {
                ...
                if (tmp.getIsChecked() != null && tmp.getIsChecked().equalsIgnoreCase("1")) {
                    ((SubmissionFileListAdapter.CustomViewHolder) holder).mCheckBox.setChecked(true);
                } else {
                    ((SubmissionFileListAdapter.CustomViewHolder) holder).mCheckBox.setChecked(false);
                }
                ((SubmissionFileListAdapter.CustomViewHolder) holder).fileName.setText(tmp.getName());
