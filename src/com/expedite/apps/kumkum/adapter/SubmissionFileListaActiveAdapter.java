 ...
 */
public class SubmissionFileListaActiveAdapter extends RecyclerView.Adapter {
    ...
    private int flag = 0;
    public SubmissionFileListaActiveAdapter(Activity context, List<CurriculumListModel.FileList> ItemList, AppCompatButton downloadMain, AppCompatButton shareMain, LinearLayout shareDownload) {
        mList = ItemList;
        mContext = context;
        ...
        try {
            if (holder instanceof CustomViewHolder) {
                ...
                if (tmp.getIsChecked() != null && tmp.getIsChecked().equalsIgnoreCase("1")) {
                    ((SubmissionFileListaActiveAdapter.CustomViewHolder) holder).mCheckBox.setChecked(true);
                } else {
                    ((SubmissionFileListaActiveAdapter.CustomViewHolder) holder).mCheckBox.setChecked(false);
                }
                ((SubmissionFileListaActiveAdapter.CustomViewHolder) holder).fileName.setText(tmp.getName());
